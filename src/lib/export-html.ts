...
const SHARED_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .message-parallax {
    ...
    background: rgba(106, 171, 142, 0.04);
  }
  ...
  @media print {
    body { background: white; color: #1a1410; padding: 20px; }
    .hero-quote { background: #f5f0e8; }
...
export function buildSessionSummaryHtml(
  ...
  if (summary.overallInsight) {
    ...
    <div class="hero-quote">
      <p>${escapeHtml(summary.overallInsight)}</p>
    </div>`
  }
  ...
  if (summary.temperatureArc) {
    body += `
    <div class="section">
      <p class="section-title">Temperature Arc</p>
      <p style="font-size:14px;color:var(--text);margin:0;line-height:1.7;">${escapeHtml(summary.temperatureArc)}</p>
    </div>`
  }
  ...
  if (summary.keyMoments.length > 0) {
    const moments = summary.keyMoments
      .map((m) => `<div class="moment-item"><div class="moment-dot"></div><p class="moment-text">${escapeHtml(m)}</p></div>`)
      .join('\n')
    body += `
    <div class="section">
  ...
  if (summary.lensInsights && summary.lensInsights.length > 0) {
    const lenses = summary.lensInsights
      .map((l) => `<div class="moment-item"><div class="moment-dot lens-dot"></div><p class="moment-text">${escapeHtml(l)}</p></div>`)
      .join('\n')
    body += `
    <div class="section">
      <p class="section-title">Lens Insights</p>
      ${lenses}
    </div>`
  }
  ...
  if (summary.resolutionTrajectory) {
    body += `
    <div class="section">
      <p class="section-title">Resolution Trajectory</p>
      <p style="font-size:14px;color:var(--text);margin:0;line-height:1.7;">${escapeHtml(summary.resolutionTrajectory)}</p>
    </div>`
  }

  // Divider before person sections
  ...
  body += `
    <div class="footer">
      <p>Generated by Parallax</p>
      <p style="margin-top:4px;font-size:8px;color:var(--muted);opacity:0.6;">Share with your therapist or keep for personal reflection</p>
    </div>`
...
function buildPersonHtml(name: string, needs: string, takeaway: string, strength: string): string {
  return `
    <div class="person-card">
      ...
      ${needs ? `
      <p class="person-field-label">What they needed</p>
      <p class="person-field-value">${escapeHtml(needs)}</p>` : ''}
      ${takeaway ? `
      <p class="person-field-label">Takeaway</p>
      <p class="person-field-value">${escapeHtml(takeaway)}</p>` : ''}
      ${strength ? `
      <p class="person-field-label">What they did well</p>
      <p class="person-field-value strength">${escapeHtml(strength)}</p>` : ''}
    </div>`
}
...
export function buildExportHtml(
  ...
  if (insights && (insights.identity?.name || insights.currentSituation || insights.themes?.length)) {
    ...
    if (insights.identity?.name) {
      body += `
      <div class="section">
        <h2 class="serif" style="font-size:20px;font-weight:400;color:var(--heading);margin:0 0 4px 0;">${escapeHtml(insights.identity.name)}</h2>
        ${insights.identity.bio ? `<p style="font-size:13px;color:var(--muted);margin:4px 0 0 0;">${escapeHtml(insights.identity.bio)}</p>` : ''}
        ${insights.emotionalState ? `<p class="mono" style="font-size:10px;color:var(--warm);margin:8px 0 0 0;">${escapeHtml(insights.emotionalState)}</p>` : ''}
      </div>`
    }
    ...
    if (insights.currentSituation) {
      body += `
      <div class="section">
        <p class="section-title">Situation</p>
        <p style="font-size:14px;color:var(--text);margin:0;">${escapeHtml(insights.currentSituation)}</p>
      </div>`
    }

    // Themes
    if (insights.themes && insights.themes.length > 0) {
      const tags = insights.themes.map((t) => `<span class="tag">${escapeHtml(t)}</span>`).join('')
      body += `
      <div class="section">
    ...
    if (insights.patterns && insights.patterns.length > 0) {
      const items = insights.patterns.map((p) => `<div class="pattern-item">${escapeHtml(p)}</div>`).join('')
      body += `
      <div class="section">
    ...
    if (insights.values && insights.values.length > 0) {
      const tags = insights.values.map((v) => `<span class="tag">${escapeHtml(v)}</span>`).join('')
      body += `
      <div class="section">
    ...
    if (insights.strengths && insights.strengths.length > 0) {
      const tags = insights.strengths.map((s) => `<span class="tag" style="border-color:var(--cool);color:var(--cool);">${escapeHtml(s)}</span>`).join('')
      body += `
      <div class="section">
